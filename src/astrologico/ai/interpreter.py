...
Integrates LLM capabilities (OpenAI, Anthropic) for intelligent chart analysis
and insights generation with caching support.
"""
...
class InterpretationCache:
    """Cache for storing AI interpretations."""
...
class AstrologicalInterpreter:
    ...
    Supports OpenAI (GPT-4) and Anthropic (Claude) backends with
    intelligent interpretation and caching.
    """
    ...
    def __init__(self, api_provider: str = "openai", api_key: Optional[str] = None):
        ...
        Args:
            api_provider: 'openai' or 'anthropic' (default: 'openai')
    ...
    def interpret_aspects(self, aspects: List[Dict[str, Any]]) -> str:
        ...
        Returns:
            Human-readable interpretation of aspects
        """
        if not aspects:
...
Provide a clear, helpful response grounded in astrological knowledge."""
    ...
    def _query_ai(self, prompt: str) -> str:
        ...
        Raises:
            ...
            ValueError: If API provider unknown
        """
        if not self.client:
        ...
        if self.api_provider == "openai":
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "You are an expert astrologer with deep knowledge of natal charts, aspects, and astrological interpretation. Provide insightful, balanced, and constructive guidance."
                    },
